...
def match_address(pos,value,save_path):
    for i in range(len(pos)):
        if '住址' in value[i] or '住' in value[i] or '址' in value[i] or '佳址' in value[i]:
            ymin=pos[i][0][1]
            ymax=pos[i][2][1]
            xmin=pos[i][0][0]
            xmax=pos[i][2][0]
            ...
            for i in range(len(pos)):
                if shr_pos[1][0]-width<pos[i][0][0]<shr_pos[1][0]+width and shr_pos[1][1]-height*7<pos[i][0][1]<shr_pos[1][1]-height:
