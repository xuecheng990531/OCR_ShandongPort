                ...
                for i in range(len(pos)):
                    if shr_pos[1][0] < pos[i][0][0] < shr_pos[1][
                            0] + width * 2 and shr_pos[0][1] - int(
                                2 * height
                            ) < pos[i][0][1] < shr_pos[2][1] + int(height / 2):
                        if '性别' in value[i]:
...
def match_shenfenzhenghao(pos, value, save_path):
    for i in range(len(pos)):
        if re.findall(id_zhengze, value[i]):
            ...
            if value[i].split('号')[1] != "" and value[i].split(
                    '号')[-1][2:8].isdigit():
                if len(value[i].split('号')[-1]) >= 15 and len(
                        value[i].split('号')[1]) <= 19:
                ...
                for i in range(len(pos)):
                    if shr_pos[1][0] - width / 2 < pos[i][0][0] < shr_pos[1][
                            0] + width * 3 and shr_pos[1][1] - height < pos[i][
...
def match_congyezigeleibie(pos, value, save_path):
    for i in range(len(pos)):
        ...
        if '性别' in value[i]:
            ...
            width = pos[i][1][0] - pos[i][0][0]
            for i in range(len(pos)):
                if shr_pos[1][0] + width * 4.5 < pos[i][0][0] < shr_pos[1][
                        0] + width * 10 and shr_pos[0][1] - int(
                            2 * height) < pos[i][0][1] < shr_pos[2][1] + int(
                                height / 2):
                    type.append(value[i])
            all = ''.join(type)
    ...
    for i in range(len(pos)):
        if '初次' in value[i]:
            if len(value[i].split('日')[-1]) > 2:
                ...
                for i in range(len(pos)):
                    if shr_pos[1][0] - int(
                            width / 4) < pos[i][0][0] < shr_pos[1][0] + int(
                                width / 4) and shr_pos[1][1] - int(
                                    height /
    ...
    for i in range(len(pos)):
        if '档案' in value[i] or '档家' in value[i] or '当案' in value[i]:
            if len(value[i]) > 5:
                ...
                width = pos[i][1][0] - pos[i][0][0]
                for i in range(len(pos)):
                    if shr_pos[1][0] - int(width / 4) < pos[i][0][0] < shr_pos[
        ...
        if '二维码' in value[i]:
            ...
            width = pos[i][1][0] - pos[i][0][0]
            for i in range(len(pos)):
                if shr_pos[0][0] - width < pos[i][0][0] < shr_pos[1][
                        0] + width and shr_pos[2][1] + height * 2.5 < pos[i][
                            0][1] < shr_pos[2][
                                1] + height * 6 and '网' not in value[i]:
