    ...
    for i in range(len(pos)):
        if '发货人' in value[i]:
            if len(value[i]) > 5:
                ...
                for i in range(len(pos)):
                    if shr_pos[1][0] < pos[i][0][0] < shr_pos[0][
                            0] + width * 5 and shr_pos[0][1] - int(
                                height / 2
    ...
    for i in range(len(pos)):
        if '收货' in value[i]:
            if len(value[i]) > 5:
                ...
                for i in range(len(pos)):
                    if shr_pos[1][0] < pos[i][0][0] < shr_pos[0][
                            0] + width * 2.5 and shr_pos[0][1] - int(
                                height / 2
                            ) < pos[i][0][1] < shr_pos[0][1] + int(height / 2):
                        return value[i]
...
def match_hangminghangci_english(pos, value, save_path):
    for i in range(len(pos)):
        if 'Voyage' in value[i]:
            ...
            for i in range(len(pos)):
                if 118 < pos[i][1][0] - pos[i][0][0] < 218 and 6 < pos[i][2][
                        1] - pos[i][0][1] < 76 and 48 < pos[i][0][
                            0] < 108 and 340 < pos[i][3][1] < 400:
                    return value[i][0]
    ...
    for i in range(len(pos)):
        if '装货港' in value[i]:
            if len(value[i].split('港')[-1]) >= 2:
                ...
                for i in range(len(pos)):
                    if shr_pos[1][0] < pos[i][0][0] < shr_pos[0][
                            0] + width * 2.4 and shr_pos[0][1] - int(
                                height / 2
                            ) < pos[i][0][1] < shr_pos[0][1] + int(height / 2):
                        return value[i]
...
def match_IMO(pos, value, save_path):
    ...
    for i in range(len(pos)):
        if '/-' in value[i]:
            if value[i].split('/-')[0].isdigit() or value[i].split(
                    '/-')[0] == '-':
                return value[i]
        ...
        if '危险类别' in value[i] and len(value[i]):
            ...
            width = pos[i][1][0] - pos[i][0][0]
            for i in range(len(pos)):
                if shr_pos[0][0] - int(width / 5) < pos[i][0][0] < shr_pos[1][
                        0] + width / 3 and shr_pos[3][1] - height < pos[i][0][
    ...
    for i in range(len(pos)):
        if '危规编号' in value[i] and 'UN' in value[i]:
            if len(value[i].split('r')[-1]) > 2:
                ...
                width = pos[i][1][0] - pos[i][0][0]
                for i in range(len(pos)):
                    if shr_pos[1][0] - int(width / 6) < pos[i][0][
                            0] < shr_pos[1][0] + width and shr_pos[1][1] - int(
                                height / 2
                            ) < pos[i][0][1] < shr_pos[3][1] + int(height / 2):
                        return value[i]
                ...
                for i in range(len(pos)):
                    if shr_pos[1][0] - int(
                            width / 6) < pos[i][0][0] < shr_pos[1][0] + int(
                                width / 2) and shr_pos[1][1] - int(
                                    height / 2) < pos[i][0][
                                        1] < shr_pos[3][1] + int(height / 2):
                        return value[i]
...
def match_shandian(pos, value, save_path):
    for i in range(len(pos)):
        if '℃' in value[i] and value[i].split('℃')[0][-1].isdigit():
            ...
            for i in range(len(pos)):
                if shr_pos[0][0] - int(
                        width / 5) < pos[i][0][0] < shr_pos[0][0] + int(
                            width / 5) and shr_pos[3][1] - int(
                                height /
    ...
    for i in range(len(pos)):
        if '应急措施编号' in value[i]:
            if 'No' in value[i] and len(value[i].split('No')[-1]) > 2:
                ...
                width = pos[i][1][0] - pos[i][0][0]
                for i in range(len(pos)):
                    if shr_pos[1][0] - int(width / 5) < pos[i][0][
                            0] < shr_pos[1][0] + width and shr_pos[1][1] - int(
                                height / 2) < pos[i][0][1] < shr_pos[3][
...
def match_baojianzhonglei(pos, value, save_path):
    for i in range(len(pos)):
        if 'packages' in value[i] and 'Bulk' not in value[i] and '/' in value[
                i] and value[i].split('/')[-1][0].isdigit():
...
def match_kongzhiwendu(pos, value, save_path):
    for i in range(len(pos)):
        if 'emergency' in value[i]:
            ...
            elif 85 < pos[i][1][0] - pos[i][0][0] < 100 and 6 < pos[i][2][
                    1] - pos[i][0][1] < 35 and 290 < pos[i][0][
                        0] < 340 and 790 < pos[i][0][1] < 830:
    ...
    for i in range(len(pos)):
        if 'POLLUT' in value[i]:
            if '是' in value[i].split('POLLUT')[-1]:
                ...
                for i in range(len(pos)):
                    if shr_pos[0][0] - int(
                            width / 3) < pos[i][0][0] < shr_pos[0][0] + int(
                                width /
                                2) and shr_pos[3][1] - height * 1.4 < pos[i][
                                    0][1] < shr_pos[3][1] + height * 1.2:
                        result.append(value[i])
                if len(result) != 0:
        ...
        if 'Total' in value[i] and 'weight' in value[i]:
            ...
            for i in range(len(pos)):
                if shr_pos[1][0] < pos[i][0][0] < shr_pos[1][
                        0] + width * 1.2 and shr_pos[1][1] - int(
                            height /
                            2) < pos[i][0][1] < shr_pos[3][1] + height:
                    result.append(value[i])
            if len(result) != 0:
